...
sha256sum -c dist/clusterweaver-0.1.8-linux-amd64-offline.tar.gz.sha256
```
...
```bash
cd /root
sha256sum -c clusterweaver-0.1.8-linux-amd64-offline.tar.gz.sha256
tar -xzf clusterweaver-0.1.8-linux-amd64-offline.tar.gz
cd clusterweaver-0.1.8-linux-amd64-offline
./preflight.sh
./install-offline.sh
```
