...
def generate_hosts_update(project: ProjectData) -> str:
    ...
    lines = [
        ...
        '$0 ~ /^# BEGIN ClusterWeaver / { in_block=1; block=$0 ORS; block_conflict=0; next }',
        'in_block { block=block $0 ORS; conflict=managed_ip[$1]; for (i=2; i<=NF; i++) if (managed_name[$i]) conflict=1; if (conflict) block_conflict=1; if ($0 ~ /^# END ClusterWeaver /) { if (!block_conflict) printf "%s", block; in_block=0; block="" } next }',
