...
def configure_node_network(node, password: str, rollback_seconds: int = 90) -> NetworkConfigResult:
    ...
    try:
        ...
        cleanup = (
            _command("nmcli", "connection", "delete", profile) + " >/dev/null 2>&1 || true; "
        ...
        add_args = [
            ...
            "con-name", profile, "connection.autoconnect", "yes", "connection.autoconnect-priority", "100",
            "ipv4.method", "manual", "ipv4.addresses", node.management_ip,
            "ipv4.gateway", node.management_gateway,
        ]
        if dns_servers:
            add_args.extend(["ipv4.dns", dns_servers])
        status, output = _run(client, _command(*add_args))
        if status != 0:
            return NetworkConfigResult(node.hostname, endpoint, False, "Could not create the management profile.\n" + output)
        ...
        schedule = _command(
            "systemd-run", f"--unit={unit}", f"--on-active={rollback_seconds}s",
            "/bin/sh", "-c", rollback_command,
        )
        ...
        activate = (
            "nohup " + _command("nmcli", "connection", "up", profile, "ifname", node.primary_interface)
            + " >" + shlex.quote(activation_log) + " 2>&1 </dev/null &"
        )
        try:
            _run(client, activate)
        except Exception:
            pass  # A management address change is expected to interrupt this SSH session.
        finally:
            client.close()
            client = None
...
"""
        ...
        return NetworkConfigResult(node.hostname, f"{new_ip}:{node.ssh_port or 22}", status == 0, "\n".join(log))
    except Exception as exc:
        log.append(str(exc))
